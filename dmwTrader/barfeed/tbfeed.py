...
class RowParser(csvfeed.RowParser):
    ...
    def parseBar(self, csvRowDict):
        dateTime = self.__parseDate(csvRowDict["时间"])
        close = float(csvRowDict["收"])
        open_ = float(csvRowDict["开"])
        high = float(csvRowDict["高"])
        low = float(csvRowDict["低"])
        volume = float(csvRowDict["成交量"])
        adjClose = None
...
class Feed(csvfeed.BarFeed):
    ...
    def __init__(self, frequency=bar.Frequency.MINUTE, timezone=None, maxLen=None):
        if frequency not in [bar.Frequency.DAY, bar.Frequency.MINUTE]:
